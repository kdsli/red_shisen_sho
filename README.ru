...
Программа не планировалась как публичная, но вполне работоспособная. При этои исходный текст программы успешно
проходит проверку:

cppcheck -q --enable=all --inconclusive --std=c++14

Программа пока не собиралась под Windows, но будет проверена в ближайшее время и проблем не должно быть. 
Также не было сделано никаких работ по установке программы в среду Linux.
